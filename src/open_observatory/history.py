...
def _withdrawn_flag() -> ColumnElement[Any]:
    """The stored withdrawal boolean, as a SQL expression (ADR-042).
...
def is_withdrawn() -> ColumnElement[Any]:
    """True for a detection whose claim has been withdrawn by review (ADR-042).
...
def species_summary(
    ...
    *,
    ...
    Excludes withdrawn detections by default for the same reason (ADR-042). This
