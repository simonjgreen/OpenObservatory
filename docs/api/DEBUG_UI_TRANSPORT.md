...
| `event` | As they occur | `{"type": "event", "event": <envelope>}` — the envelope is **nested under `event`**, not spread into the frame. The envelope itself is exactly as specified in [[API_AND_INTEGRATIONS]] |
