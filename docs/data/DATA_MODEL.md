...
## Migrations (Alembic, ADR-035, wired up by ADR-041)
...
The live station reports `0004_drop_dead_detection_indexes (head)` — confirmed
...
`alembic check` reports no drift, row counts unchanged).
...
**As of ADR-041, Alembic is the only thing that creates or changes schema.**
...
ADR-041 for why the migration itself runs in the deploy script and not in
application startup.
...
- Station SQLite: `deploy/deploy.sh` runs `alembic upgrade head` itself, as
  an explicit step before the systemd unit is restarted (ADR-041) — a normal
...
**If `deploy/deploy.sh`'s migration step itself fails (ADR-041):** the script
