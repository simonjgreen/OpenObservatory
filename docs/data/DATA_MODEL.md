...
Authentication now exists ([[ADR-034 - Authentication foundation|ADR-034]], closing [[ADR-015 - Anonymous read, auth deferred|ADR-015]]) but is **off by default**,
...
- last_frame_at_utc — nullable; a heartbeat written every ~10 s while the
  stream is open, added by [[ADR-024 - Coverage bounded by frames|ADR-024]]. NULL for rows written before it existed
...
- detail — free-form JSON; also now carries an `orphan_recovery` or
  ...
  the original claim and the method used ([[ADR-024 - Coverage bounded by frames|ADR-024]]).

**Correctness note ([[ADR-024 - Coverage bounded by frames|ADR-024]]):** `start_utc`/`end_utc` is a *claim*, not
...
- refined_at (indexed) / refinement_version / refinement_outcome — added by
  revision `0006_refinement` ([[ADR-045 - Refinement runner|ADR-045]]); described in full in the refinement
  section below

**`native_result` since 2026-08-09 ([[ADR-037 - Prune the dead indexes|ADR-037]] option C)**: `normaliser.py`
...
`occurrence_probability` and `plausibility_band` ([[ADR-032 - Plausibility bands|ADR-032]]'s plausibility
...
reconstruct them. See [[ADR-037 - Prune the dead indexes|ADR-037]]'s "B and C: what was implemented" section for
...
actually in use has also grown: `audible_ultrasonic` (see [[ADR-014 - Ultrasonic rendered audible|ADR-014]] and
...
- reclaimed_at nullable — set by the retention sweeper ([[ADR-026 - Tiered clip retention|ADR-026]]) when it
...
`POST /api/v1/detections/{id}/review` ([[ADR-029 - Retention UI, assumed API|ADR-029]]) inserts a row on every call —
...
**The correction columns are now written ([[ADR-043 - Taxon correction|ADR-043]], 2026-08-09).** This section
...
No shipped refiner may set `applied`. See [[ADR-045 - Refinement runner|ADR-045]] for the measured accuracy
...
process* on a paced 1.5 s budget ([[ADR-026 - Tiered clip retention|ADR-026]], [[ADR-033 - Retention is paced|ADR-033]]), so that question has to be
...
consult these columns** — see [[ADR-045 - Refinement runner|ADR-045]]'s "What this does not do" for the exact
...
[[ADR-052 - Near-miss ledger|ADR-052]] records what BirdNET proposed and refused, with per-band score
...
the detection table's growth problem ([[ADR-037 - Prune the dead indexes|ADR-037]]) into a table nobody browses,
against a charter cross-cutting constraint on SD-card write amplification. If
you find yourself adding a migration for it, read [[ADR-052 - Near-miss ledger|ADR-052]] first.
...
- `refinement (detection_id, evidence_fingerprint)` — **unique**; this is charter
  item 5's "only from new information" rule as a database constraint ([[ADR-045 - Refinement runner|ADR-045]])
...
**Dropped 2026-08-09 ([[ADR-037 - Prune the dead indexes|ADR-037]] option B, Alembic revision
...
`plausibility_repair.reconcile_plausibility` ([[ADR-032 - Plausibility bands|ADR-032]]) joins from `detector`
...
- evidence clips: **tiered aging, not a flat 30 days** ([[ADR-026 - Tiered clip retention|ADR-026]],
...
- **an open gap, recorded rather than closed ([[ADR-045 - Refinement runner|ADR-045]]):** the clip tiers above
...
| `0003_auth_tables` | `user`, `auth_session`, `api_token` ([[ADR-034 - Authentication foundation\|ADR-034]]) |
| `0004_drop_dead_detection_indexes` | drops four unused `detection` indexes found dead by re-verification against the live database ([[ADR-037 - Prune the dead indexes\|ADR-037]] option B) |
| `0005_review_correction_names` | `review.corrected_common_name` / `corrected_scientific_name`, so a human correction carries the name it asserts ([[ADR-043 - Taxon correction\|ADR-043]]) |
| `0006_refinement` | the `refinement` table plus `detection.refined_at` / `refinement_version` / `refinement_outcome` and `ix_detection_refined_at` ([[ADR-045 - Refinement runner\|ADR-045]]) |

**The live station is at `0006_refinement`, which is `head`** — read directly
from its `alembic_version` table, read-only, on 2026-08-09 at 21:46Z. [[ADR-042 - Migrations run in deploy.sh|ADR-042]]'s
...
**As of [[ADR-042 - Migrations run in deploy.sh|ADR-042]], Alembic is the only thing that creates or changes schema.**
...
[[ADR-042 - Migrations run in deploy.sh|ADR-042]] for why the migration itself runs in the deploy script and not in
application startup.
...
- **Existing database built by `create_all()` before Alembic existed** (a
  ...
  this position until it was adopted while [[ADR-035 - Alembic environment|ADR-035]] was written): **do not
...
- Station SQLite: `deploy/deploy.sh` runs `alembic upgrade head` itself, as
  an explicit step before the systemd unit is restarted ([[ADR-042 - Migrations run in deploy.sh|ADR-042]]) — a normal
...
**If `deploy/deploy.sh`'s migration step itself fails ([[ADR-042 - Migrations run in deploy.sh|ADR-042]]):** the script
