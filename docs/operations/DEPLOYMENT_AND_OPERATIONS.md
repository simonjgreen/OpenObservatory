...
target — see [[ADR-007 - SQLite in developer mode|ADR-007]] and [[ADR-008 - systemd, not Compose|ADR-008]] in [[ADRS]] for why the two
...
`systemd`. Storage is SQLite by default ([[ADR-007 - SQLite in developer mode|ADR-007]]); there is no database
server. There is no message broker; the event bus is in-process ([[ADR-009 - In-process event bus|ADR-009]]).
...
4. runs `alembic upgrade head` **before** anything is restarted, against the
   database the still-running old version is using ([[ADR-042 - Migrations run in deploy.sh|ADR-042]]). A slow or failing
...
5. installs `deploy/open-observatory.service`, the refinement service and its
   timer ([[ADR-045 - Refinement runner|ADR-045]]), and `deploy/99-audiomoth.rules`; reloads `udev` and
...
([[ADR-006 - Model install and licensing|ADR-006]] — model licences differ from the code's and are not bundled).
...
It is a separate process on purpose, and the fence is the point. [[ADR-033 - Retention is paced|ADR-033]]
...
| `oo audio resample-check` | verify group delay, delivery-latency bounds and seam continuity. Runs in constant memory, so `--seconds 3600` — the duration the audio pipeline spec asks for, and [[ADR-069 - Two drift gates\|ADR-069]] gate (a) — costs about as much resident memory as `--seconds 60`. `--source-rate`, `--target-rate`, `--seconds`, `--block-ms`, `--json` |
...
| `oo history reconcile-streams` | repair `audio_stream` rows whose `end_utc` is a claim the frame count contradicts ([[ADR-024 - Coverage bounded by frames\|ADR-024]]). **Dry-run by default**; `--apply`, `--yes`, `--json`, `--ratio-threshold` |
| `oo detections reconcile-plausibility` | re-evaluate stored BirdNET detections against the current range model and plausibility floor ([[ADR-032 - Plausibility bands\|ADR-032]]). **Dry-run by default**; `--apply`, `--yes`, `--json`, `--limit`. Never deletes a row or overwrites `native_result`; adds a `native_result.plausibility_review` block. Since [[ADR-044 - Withdrawn detections\|ADR-044]] `--apply` takes effect immediately with no restart: flagged rows are marked `withdrawn` by the API, dropped from species tallies, and shown by neither MQTT nor the counter-top display |
| `oo detections reconcile-taxonomy` | stop stored sound categories (Engine, Human vocal, Dog, …) claiming to be birds at species rank ([[ADR-049 - Sound categories are not species\|ADR-049]]). **Dry-run by default**; `--apply`, `--yes`, `--json`, `--limit`. Clears `rank`, `scientific_name` and `canonical_taxon_id`, sets `taxonomic_group` to `acoustic_event`, keeps `common_name`, never deletes a row; the originals are preserved under `native_result.taxonomy_review` |
| `oo clips purge-human-audio` | delete evidence clips of human speech and mark their `media_asset` rows reclaimed ([[ADR-049 - Sound categories are not species\|ADR-049]]). **Dry-run by default**; `--apply`, `--yes`, `--json`. Detection rows are never touched |
| `oo clips reconcile-missing` | reconcile `media_asset` rows that claim a clip the disk does not have ([[ADR-057 - Evidence rows must be checkable\|ADR-057]]). **Dry-run by default**; `--apply`, `--yes`, `--json`, `--limit`. Sets `reclaimed_at` and `reclaim_reason = "missing"` — never a retention tier name, because nothing decided to give these clips up — and preserves what the row claimed under `detail.missing_reconciliation`. Deletes no file, no `media_asset` row and no `detection`. The station reports the same condition on its own as a health note and as `oo_media_missing_files`; a non-zero figure there is what should prompt this command |
| `oo clips retention` | run the tiered retention sweep manually ([[ADR-026 - Tiered clip retention\|ADR-026]]). `--dry-run`, `--limit` |
| `oo refine run` | one refinement pass over stored evidence clips ([[ADR-045 - Refinement runner\|ADR-045]]). Normally started by `open-observatory-refine.timer`, not by hand. Refuses outside 01:00–03:00 UTC unless `--force`. `--dry-run`, `--limit`, `--json`. **Writes only append-only `refinement` rows plus three bookkeeping columns; never edits a detection's species, score or `native_result`** |
...
A display running pre-[[ADR-050 - Display OTA slots|ADR-050]] firmware reports its version as `unknown`. That is
...
Put it in a quiet window — **not** dawn, dusk or bat hours; [[ADR-067 - Unattended package work|ADR-067]]'s 15:00 slot
...
own stream row ([[ADR-066 - Graceful shutdown closes the row|ADR-066]]), so it appears as a clean end rather than an unclean
one.

### Read `capture_integrity_ratio`, not `continuity_ratio` (ADR-073)

`continuity_ratio` sums two unrelated things: audio genuinely lost, and the
microphone crystal's drift (ADR-072). `capture_integrity_ratio` counts only
audio that is actually gone.

On a healthy station integrity reads **1.0** while continuity sits near 0.99995,
and the whole difference is the crystal. Measured 14 seconds after a restart on
2026-08-29, continuity read **0.995** — a figure that invites somebody to go
hunting for a 0.5% loss that does not exist — while integrity correctly read
**1.0** with `audio_lost_seconds` at 0.0.

| key | means |
|---|---|
| `capture_integrity_ratio` | SLO B. Of the audio that should have been recorded while capture was live, the fraction that was. Blind to drift. |
| `audio_lost_seconds` | confirmed lost audio, **this stream** |
| `drift_seconds` | timestamp error from the crystal. Accepted, not loss. |
| `continuity_ratio` | kept for continuity with older records; it is the sum of the two above |
| `estimated_missing_seconds` | confirmed loss for the whole **process**, across stream reopens |

The same split is on `/metrics` as `oo_capture_integrity_ratio`,
`oo_capture_audio_lost_seconds` and `oo_capture_drift_seconds`.
...
database and restart — see the [[ADR-055 - Timed recording pause|ADR-055]] rollback section in
...
explained ([[ADR-019 - Chunked-WAV live playback|ADR-019]]) — media-element playback worked on the same machine where Web
...
documented production DSN target. As of [[ADR-035 - Alembic environment|ADR-035]]:
...
- `src/open_observatory/db/session.py: create_all()` **no longer runs at
  startup**. [[ADR-042 - Migrations run in deploy.sh|ADR-042]] replaced it with `ensure_schema_at_head()`, which
  bootstraps an empty database and otherwise refuses to start against one that
  is unstamped or behind, and it deleted the `ALTER TABLE` patcher outright.
  `create_all()` survives for tests — and, as of 2026-08-29, at two call sites
  it should not be at: `oo refine run` and `oo refine status`
  (`src/open_observatory/cli.py:1661`, `:1744`), the first of which is the
  `ExecStart` of the nightly `open-observatory-refine.service`. New columns go
  through an Alembic revision (full reasoning in [[ADR-035 - Alembic environment|ADR-035]] and [[ADR-042 - Migrations run in deploy.sh|ADR-042]]);
...
**`deploy.sh` runs `alembic upgrade head` on every deploy** ([[ADR-042 - Migrations run in deploy.sh|ADR-042]]), after the
...
- `redis:7-alpine` with append-only persistence, matching the "Redis Streams
  ...
  implemented ([[ADR-009 - In-process event bus|ADR-009]] describes only the in-process `EventBus`, with Redis
...
[[ADR-008 - systemd, not Compose|ADR-008]] records why this was deferred rather than built first: only the
...
**Since [[ADR-048 - Web-configurable settings|ADR-048]], the browser is the primary way to configure a station.**
...
6. `birdnet_threshold_out_of_range` and `birdnet_plausibility_floor` — for
   implausible species rather than noise ([[ADR-032 - Plausibility bands|ADR-032]]).
...
| `retention_audible_only_days` | live (pushed) | `30` days | keep every audible clip for, unless kept ([[ADR-061 - Operator keep flag\|ADR-061]]) |
...
MQTT publisher shipped ([[ADR-025 - MQTT and Home Assistant|ADR-025]]). Re-checked mechanically on 2026-08-09: **every
...
password (see [[ADR-034 - Authentication foundation|ADR-034]]'s bootstrap note).
...
gap [[ADR-015 - Anonymous read, auth deferred|ADR-015]] recorded as a "real security consequence." It does **not**
...
on the LAN, same as [[ADR-015 - Anonymous read, auth deferred|ADR-015]] always implied.

**The ESP32 counter-top display's exemption.** `firmware/inside-observer` reads
`/api/v1/display` (its WebSocket push channel since [[ADR-038 - Display push channel|ADR-038]]) and, when that is
down, polls `GET /api/v1/detections` and `GET /api/v1/health` — with no way to
carry a credential. (It could not be reflashed without a USB cable when
[[ADR-034 - Authentication foundation|ADR-034]] was written; [[ADR-050 - Display OTA slots|ADR-050]] gave it OTA slots, and the exemption was
kept deliberately anyway — an update path that only works while auth is off is
one that stops working on the day it matters.) Those paths (plus `GET /metrics`, scraped by Prometheus with no
...
trough. [[ADR-067 - Unattended package work|ADR-067]] moves both to 15:00 with 15 minutes of jitter and
`Persistent=false`.
...
- **that nothing else scheduled ran inside the window** — the host's
  `apt-daily*` timers ([[ADR-067 - Unattended package work|ADR-067]]), `open-observatory-refine.timer` at 02:02,
  and any deploy. A restart voids the run, and until [[ADR-065 - Unclean restart is reported|ADR-065]] nothing said
