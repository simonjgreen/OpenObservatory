...
See ADR-022 in `docs/architecture/ADRS.md` for why it is built this way
...
- **This integration never reads anything from Home Assistant.** It is
  ...
  *into* the station) and the alert rule engine are not implemented yet — see
  `IMPLEMENTATION_PLAN.md`'s Milestone 6 scope and ADR-022.
...
than a per-species entity (there isn't one, deliberately — see ADR-022):
