...
This document records the failure as a failure. [[ADR-069 - Two drift gates|ADR-069]] says a threshold chosen
after seeing the numbers is not a threshold, so nothing below moves one. The
proposal at the end is a proposal.

## The run

    python3 scripts/measure_capture_drift.py --host <station-host> --port 8080 \
...
| declared load | post-soak; station idle apart from normal capture and detection; display connected on 0.2.5; no deploy, no ssh, no other agent. **Unprotected mains** ([[ADR-069 - Two drift gates\|ADR-069]] requires this be stated) |
...
[[ADR-069 - Two drift gates|ADR-069]] sets the 2 ppm agreement threshold explicitly so that it sits "above the
...
produces. [[ADR-069 - Two drift gates|ADR-069]] already notes these two quantities have different anchors —
"a slope against a cumulative average" — and that they are complementary rather
than corroborating.

## What the run does establish

[[ADR-069 - Two drift gates|ADR-069]] states the purpose of this gate plainly: the failed soak left ~175 s of
...
The linearity and step thresholds (0.5 ms) were derived from [[ADR-046 - Deficit is mostly drift|ADR-046]]'s three
...
— which is the specific thing [[ADR-069 - Two drift gates|ADR-069]] lengthened the window to expose.
...
fail on an old one.** [[ADR-069 - Two drift gates|ADR-069]] anticipated the direction of this — it notes the
