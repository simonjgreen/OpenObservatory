...
denying it the chance to run. This is the same shape as [[ADR-020 - Non-live sources excluded|ADR-020]]'s incident (§3a),
...
**Reviewed 2026-08-29:** the decision holds and is deployed, with all four parts
in the tree — `stall_timeout_s` and its `_no_progress` bound on both branches
(`src/open_observatory/audio/alsa_source.py:82`, `:340`), the ALSA-only
`asyncio.wait_for` (`src/open_observatory/station.py:900`), and the `deaf` term
in the severity expression (`src/open_observatory/api/app.py:1195`) — and the
station serves both settings at the defaults named above. **The second open item
is closed:** [[ADR-061 - Operator keep flag|ADR-061]] removed the retention sweep's exemplar query, and no gap
arrived on the `retention_interval_s` beat across the 72-hour window of
2026-08-22 to 2026-08-25 ([[SOAK_2026-08-22]]). The wedge itself is still
unexplained at the device level. What changed is the thing that was driving the
device restarts, not the understanding of why one of them never came back; the
station has not wedged since, so nothing has exercised these bounds in the field.
