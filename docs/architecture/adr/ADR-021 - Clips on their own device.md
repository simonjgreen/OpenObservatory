...
**Status:** active; two supporting facts have moved since it was written. The ALSA
read no longer shares the default thread pool described below —
[[ADR-030 - ALSA ring and capture thread|ADR-030]] gave it its own single-thread
executor, citing this ADR's evidence executor as the precedent. The clip-directory GB
budget in the Consequence is no longer what bounds the archive —
[[ADR-026 - Tiered clip retention|ADR-026]] moved housekeeping onto `RetentionSweeper`
and its 85% watermark, and nothing calls `ClipManager.enforce_retention` in the
running station.
...
> **Not the cause of the missing clips ([[ADR-057 - Evidence rows must be checkable|ADR-057]]).** 8,067 `media_asset` rows
