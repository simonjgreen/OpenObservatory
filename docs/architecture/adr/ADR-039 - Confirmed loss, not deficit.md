...
**Status:** active. Closes the open item in [[ADR-033 - Retention is paced|ADR-033]] and amends [[ADR-030 - ALSA ring and capture thread|ADR-030]]'s
estimator; the question it left open is answered by [[ADR-046 - Deficit is mostly drift|ADR-046]]. Confirmed on
target 2026-08-09 — see the review note at the end, which corrects this ADR's own
closing paragraph.
...
still has its cost estimated — that is [[ADR-030 - ALSA ring and capture thread|ADR-030]]'s regression and it is asserted by
...
wait. [[ADR-030 - ALSA ring and capture thread|ADR-030]] widened the ring to 500 ms precisely so that stalls would be
...
**Reviewed 2026-08-29 — the paragraph above is out of date, and is kept as
written.** The change was deployed later on 2026-08-09, incidentally to the
[[ADR-040 - Spectrograms only when watched|ADR-040]] work, and both halves were then confirmed on target: the
absorbed-stall case against the pass criteria this ADR set itself, and — the same
evening — the unabsorbed case that had only ever been proven against
`RingedDevice`, where twelve real ALSA overruns over a 4.03 h run left the
estimate and the drift-corrected deficit agreeing to 0.08 s. Both readings, with
their cautions, are in [[OPEN_INVESTIGATION_CAPTURE_GAPS]]. The estimator itself
has not changed since; whether its confirmation window is too permissive is
answered by [[ADR-046 - Deficit is mostly drift|ADR-046]].

**One consequence this ADR did not anticipate.** [[ADR-073 - Five capture SLOs|ADR-073]] makes the
confirmed-loss figure the *definition* of lost audio: capture integrity is
`1 - estimated_missing_frames / expected_frames`, and everything else in the
deficit is drift by residual (`split_deficit` in `src/open_observatory/slo.py`).
The under-report bound stated above — one ALSA period, 10 ms per event — is
therefore also the bound on how much real loss that SLO can classify as drift.
