...
**Status:** active; deployed 2026-08-10 and its first verification **failed** —
see the dated status paragraph in the body and the 2026-08-29 re-verification
beneath it.
...
instrument (added by [[ADR-033 - Retention is paced|ADR-033]]) names the phase in the same millisecond:
...
**Why chunking rather than a thread.** [[ADR-033 - Retention is paced|ADR-033]] settled this and it is the same
...
exactly [[ADR-033 - Retention is paced|ADR-033]]'s argument. It is left at 30 s so that this change moves one
...
cause. [[ADR-061 - Operator keep flag|ADR-061]] (2026-08-14) has since removed that cost — see its entry for
the re-verification.

**Reviewed 2026-08-29:** in force, and the pass criterion is now met — but the
pointer above leads nowhere, because [[ADR-061 - Operator keep flag|ADR-061]]'s own status still reads
"on-station verification pending" and records no re-verification. Read from the
station instead, 24.7 h into a run: `late_read_max_frames` **94,374 of 192,000
(49%)** across **5** late reads, with `snapshot_phase_s` giving `storage`
**0.000 s** on a **241,404-file** archive — close to the ~250,000 files this
document projected a 2.7 s walk for, and the case it was written against.
`clip_usage_age_s` read 9.0 s, so the chunked refresh is landing on its 30 s
beat. The 98.4% reading above stands as recorded; this only supplies the
