...
**Status:** active; frontend only. The ladder is still four widths and nothing clips
horizontal overflow, in the tree or in the bundle the station is serving. One deliberate
exception to "nothing is hidden to make it fit" was added afterwards — see the review
note.
...
**Constraint — nothing is hidden to make it fit ([[ADR-016 - Debug UI is the dashboard's foundation|ADR-016]], [[ADR-028 - One depth toggle|ADR-028]]).** A stat that
...
**Reviewed 2026-08-29:** the decision holds. `web/src/styles.css` still carries exactly
four width breakpoints — 640, 950, 1100 and 1500 px (`web/src/styles.css:1012`, `:457`,
`:768`, `:453`) — plus one `prefers-reduced-motion` query; no rule on `html` or `body`
clips horizontal overflow; and `.topbar-right` and `.listen` carry `flex-wrap: wrap` and
`min-width: 0` at every width (`web/src/styles.css:189-198`, `:206-211`). Checked in the
bundle the station is actually serving and not only in the tree: `assets/index-DxycfIIe.css`
from the station has the same four breakpoints and no `overflow-x: hidden` in 32 kB of
CSS. `web/src/responsive.test.tsx` passes, and now holds nine assertions rather than
eight.

Two things changed after this was written, both on the same day.

The wall clock **is** hidden below 640 px (`web/src/styles.css:1038`). That is an
exception to the constraint above, and it is written down as one where it happens: the
phone's own status bar shows the time two centimetres higher, so the information is
duplicated rather than lost. It is the only element the ladder hides for room —
`.hero-controls .grow` is a spacer with nothing in it.

And "not verified on a real iOS browser" paid out within two hours. The pause menu used
`right: 0`, so it grew leftwards from a control that is deliberately first in the header,
and on a phone it opened off the left edge of the screen. An actual iPhone found it; all
30 headless states missed it, because every capture had the menu shut. The fix anchors it
to the leading edge with `max-width: calc(100vw - 24px)` (`web/src/styles.css:1333-1342`)
and is the ninth assertion in the regression file. A sweep of static states says nothing
about a control that has to be opened first.
