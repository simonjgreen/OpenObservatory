...
- [[ADR-063 - Clock re-anchor|ADR-063]]'s clock re-anchor gets a note on the same terms, for the same reason:
...
- Combined with [[ADR-063 - Clock re-anchor|ADR-063]], the two failures found in this session share a shape
...
**Reviewed 2026-08-29:** the decision holds and is running on the station.
`_close_orphaned_streams` sets the flag, the recovered-row count and the last
audio moment (`src/open_observatory/station.py:1707`); `/api/v1/health` carries
the note in `notes`, not `problems` (`src/open_observatory/api/app.py:1108`);
`oo_station_unclean_restart` is exported as a gauge
(`src/open_observatory/api/metrics.py:118`). One thing above is misleading on
its own: "an open row at startup *is* proof that the previous run did not shut
down cleanly" was not true when it was written. No graceful shutdown had ever
closed its own row, so the note fired after a completely clean `systemctl
restart` two hours later. [[ADR-066 - Graceful shutdown closes the row|ADR-066]] made the premise true; `end_reason`
separates `station_stopped` from `process_exited` only from 2026-08-19 onwards,
and an earlier row saying `process_exited` is not evidence of a crash.
