...
**Reviewed 2026-08-29:** the staged kernel named in the last bullet has since
been booted — the station reports `Linux 6.8.0-1061-raspi` at `host.kernel` in
`/api/v1/system`. The bullet stays because its general point did not change: a
staged kernel waits for a manual reboot, and the next one will too.
