...
**Status:** active; deployed, and all three pass criteria below verified on
the station 2026-08-29 (see the review note at the end). Removes
`RetentionSweeper._exemplar_detection_ids()`; supersedes [[ADR-026 - Tiered clip retention|ADR-026]]'s
first/best-of-species exemption. Three addenda follow: the third retired the
90-day tier, so the body above it counts four tiers where three now run.
**Plan and spec:** [[2026-08-14-keep-flag-retention]],
[[2026-08-14-keep-flag-retention-design]]
...
3. **The 2026-08-14 wedge.** Each overrun forces an `snd_pcm_prepare()`
   ...
   leaving the microphone deaf for **3 h 35 min** ([[ADR-060 - A stalled read is a dead stream|ADR-060]], [[HANDOVER]]
   ...
   the length of the sweep itself. [[ADR-060 - A stalled read is a dead stream|ADR-060]] made the wedge survivable (bounded
...
**Why kept survives the watermark, and what that costs.** [[ADR-026 - Tiered clip retention|ADR-026]]'s
...
sweep's convenience. `held` ([[ADR-043 - Taxon correction|ADR-043]]) is unchanged and deliberately narrower
...
widening [[ADR-043 - Taxon correction|ADR-043]]'s watermark behaviour is a separate decision. An operator
...
curl -s http://<station-host>:8080/metrics | grep -E "oo_retention_(files_deleted|kept_detections|preamble_seconds)"
curl -s http://<station-host>:8080/api/v1/retention/status | python3 -m json.tool
...
everything behind it stopped: stream heartbeats, the [[ADR-057 - Evidence rows must be checkable|ADR-057]] media audit, the
[[ADR-059 - Clip archive measured off-loop|ADR-059]] disk-usage refresh. `clip_usage_age_s` climbed 1:1 with wall clock and
`housekeeping_blocking_s` was byte-identical between samples — a stopped clock,
not a slow one. Capture was untouched, because it owns its own thread ([[ADR-030 - ALSA ring and capture thread|ADR-030]]).
...
This was true before [[ADR-026 - Tiered clip retention|ADR-026]]'s `kept` predicate existed too, but it did not
...
**Reviewed 2026-08-29.** The decision holds and the station is running it.
All three pass criteria above now pass, fifteen days after the third
addendum's deploy:

1. `oo_retention_files_deleted_total{tier="native"}` reads 6,592, and the
   tier labels are `native`/`unkept`/`watermark` -- the third addendum's
   rename is deployed and `expired` is gone.
2. `last_preamble_s` 0.0028 against a 1.5 s budget, `last_tiers_skipped`
   empty, and `/api/v1/retention/status` reports `enabled: true`, so the
   first addendum's interim `retention_enabled=false` has been reverted.
3. The ~309 s `capture_gap` pair beat stops at 2026-08-14T12:24:11Z and has
   not recurred: every gap since (`/api/v1/gaps`) is days from its
   neighbour, not five minutes.

Two statements above are now wrong, left in place rather than rewritten:

* The `held` paragraph's "it exempts the three age-based tiers" counted the
  tiers as they stood before this ADR's own third addendum removed
  `_strip_expired`. There are **two** -- `_TIER_ORDER = ("native", "unkept",
  "watermark")`, and `held_ids` is passed to `_strip_native` and
  `_strip_unkept` only. `held`'s behaviour is unchanged; only the count is
  wrong.
* The first addendum's "**Still open** -- the honest fix is a statement
  timeout" was closed the same day it was written. The sweeper arms
  `sqlite3.Connection.set_progress_handler` around each tier's statement and
  records `RetentionReport.interrupted_tier`/`interrupted_after_s` when one
  is aborted, so a slow statement now degrades to fewer deletions instead of
  running to completion past the budget.

The rollback and smoke-test commands here named the operator's host and
