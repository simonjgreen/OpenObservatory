...
**Status:** active; resolves the open question in [[ADR-039 - Confirmed loss, not deficit|ADR-039]] and amends
[[ADR-030 - ALSA ring and capture thread|ADR-030]]'s presentation. The 2026-08-14 caveat below is closed by later
evidence: the decomposition holds at 72 hours as well as at one. See the
2026-08-29 note.
...
estimator's confirmed loss ([[ADR-039 - Confirmed loss, not deficit|ADR-039]]). The deficit is shown separately as
...
4. **Lost audio**, which is what `estimated_missing_frames` measures — being, by
   [[ADR-039 - Confirmed loss, not deficit|ADR-039]]'s construction, exactly the part of the deficit that never came back.
...
estimator is missing, and **[[ADR-039 - Confirmed loss, not deficit|ADR-039]]'s confirmation window is not too permissive**
on this evidence.
...
**Reviewed 2026-08-29: that caveat is closed, and the reading it recommended
was the right one.** The residual was real loss, not a hole in the
decomposition. It belongs to the era [[ADR-073 - Five capture SLOs|ADR-073]] measures at 662.6 s lost
across 2,983 gap events before [[ADR-060 - A stalled read is a dead stream|ADR-060]] and [[ADR-061 - Operator keep flag|ADR-061]], against 0.60 s in
the ten days since 2026-08-19. Two later soaks supply the long run this ADR
asked for, and the decomposition holds at that duration: 72.1 h, shortfall
13.5 s = 0.597 s of confirmed loss plus ~12.9 s of drift; 78.7 h, shortfall
16.0 s, drift entire, nothing lost ([[ADR-073 - Five capture SLOs|ADR-073]]).
[[DRIFT_GATE_B_2026-08-25]] separately excludes a continuous mechanism of the
~890 ppm that 175 s over 54.7 h would have required, by about nineteen times.
Read live on 2026-08-29 at 24.7 h uptime: deficit 5.061 s = 4.480 s drift +
0.534 s confirmed loss + 0.046 s residual. [[ADR-072 - Accepted crystal drift|ADR-072]] accepts the crystal
offset as measured; [[ADR-073 - Five capture SLOs|ADR-073]] generalises this ADR's rule, excluding drift
from every loss SLO and budgeting it under timestamp accuracy instead.
...
has published since [[ADR-039 - Confirmed loss, not deficit|ADR-039]] and the UI ignored — `estimated_missing_seconds`,
