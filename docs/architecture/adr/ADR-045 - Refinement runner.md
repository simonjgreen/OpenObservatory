...
**Status:** active; promotes [[ADR-017 - BatDetect2 as an optional adapter|ADR-017]]'s offline cascade to a scheduled job
without adopting it as a source of station claims. Reviewed 2026-08-29: the
decision holds unchanged — the fence, the propose-only ceiling and the untouched
retention sweeper are all still what the code does. Three of the references it
makes in passing have moved since; each carries a dated note below.
...
`IOSchedulingClass=idle`. Its first job is the [[ADR-017 - BatDetect2 as an optional adapter|ADR-017]] BatDetect2 cascade over
...
**Reason — the fence.** [[ADR-033 - Retention is paced|ADR-033]] measured what "expensive work, isolated on its
...
Zero capture gaps in either window, against [[ADR-033 - Retention is paced|ADR-033]]'s ~1.9 per minute when the
...
deliberately not used: it over-reported by 12.9× ([[ADR-039 - Confirmed loss, not deficit|ADR-039]], SETUP.md trap 10).
...
pass figure is [[ADR-017 - BatDetect2 as an optional adapter|ADR-017]]'s, from 2026-08-05. What is measured here is the *fence* —
...
The speed case for the cascade was settled by [[ADR-017 - BatDetect2 as an optional adapter|ADR-017]]'s 2026-08-05 update — 2.1 s
...
score on a species absent from the continent that [[ADR-032 - Plausibility bands|ADR-032]] ruled on: *a confident
...
- **It does not change `retention.py`.** The sweeper still deletes on age alone:
  ...
  **Reviewed 2026-08-29:** the gap is still open — nothing in `retention.py`
  reads `refined_at` — but the sweeper described here has been rebuilt around
  it. `_strip_non_exemplar` is now `_strip_unkept`
  (`src/open_observatory/retention.py:727`) and `_strip_expired` is gone:
  [[ADR-061 - Operator keep flag|ADR-061]] replaced the computed exemplar rule with an operator-set `kept`
  flag and retired the 90-day tier, so the tiers are 7 days, 30 days and the
  disk watermark, not 7, 30 or 90. [[ADR-062 - Retention walks live assets|ADR-062]] then moved tier age off
  `detection.event_start_utc` onto `media_asset.created_at`. Neither change
  affects the guard proposed above: both age tiers still join `detection`, so it
  is still one predicate per tier. Both pinning tests still exist.
- **It does not implement the second safeguard**, the explicit human hold, and it
  ...
  ([[ADR-029 - Retention UI, assumed API|ADR-029]]), which corrects the charter's "nothing writes to it yet" — but that
...
- **It does not surface refinement in the API, the UI, the MQTT publisher or the
  ...
  **Reviewed 2026-08-29:** still true of refinement *results* — no route in
  `src/open_observatory/api/app.py` reads the `refinement` table, and
  `oo refine status` is still the only place a proposal appears. The refiner's
  *settings* are a different matter: [[ADR-048 - Web-configurable settings|ADR-048]] put all nine `refinement_*`
...
columns on the next start ([[ADR-035 - Alembic environment|ADR-035]]'s known coupling), so a full schema rollback
also means reverting the code.

**Reviewed 2026-08-29: do not run that downgrade.** It named the wrong target
when it was written and the consequences have grown since. `0006_refinement`'s
`down_revision` is `0005_review_correction_names`, not
`0004_drop_dead_detection_indexes`
(`alembic/versions/20260809_0005_000000000006_refinement.py:58`); the migration
file's own docstring header still says `0004`, which is where this line came
from. Head is now `0011_retention_live_asset_indexes`, so on a current station
`downgrade 0004` would additionally revert `0005_review_correction_names`,
`0007_capture_pause`, `0008_detection_kept` — dropping
`detection.kept_at`/`kept_by` and with them the record of every clip an operator
has marked keep ([[ADR-061 - Operator keep flag|ADR-061]]) — and the two `kept_at` index revisions. The
paragraph above is stale in the other direction: [[ADR-042 - Migrations run in deploy.sh|ADR-042]] retired the ALTER
TABLE patcher (`_patch_sqlite_columns` is gone from the tree) and `create_all()`
only ever issues `CREATE TABLE` for tables that do not exist, so nothing re-adds
the three `detection` columns. What would happen instead is that
`open-observatory.service` refuses to start at all: it calls
`ensure_schema_at_head()` (`src/open_observatory/db/session.py:110`), which
raises against a database behind head. `oo refine run` is the exception — it
still calls `create_all()` (`src/open_observatory/cli.py:1661`, and
`oo refine status` at `:1744`), so the refiner alone would start, and would
re-create an empty `refinement` table without its `detection` columns. That is
