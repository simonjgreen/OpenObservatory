...
**Reviewed 2026-08-29:** the decision holds and is what the station runs.
`Station._reanchor_clock_if_stepped` (`src/open_observatory/station.py:1829`) is
called from the housekeeping tick every 10 s
(`src/open_observatory/station.py:1893`), the threshold is still 1 s
(`_CLOCK_STEP_THRESHOLD_NS`, `src/open_observatory/station.py:113`),
`After=time-sync.target` is on the unit without the blocking dependency
(`deploy/open-observatory.service:8`), and the ten tests in
`tests/test_clock_reanchor.py` pass. Two things above should not be taken
further than they go. A corrected anchor is not a correct timestamp: [[ADR-072 - Accepted crystal drift|ADR-072]]
later recorded a second frame-to-UTC naming error of the same shape — the
AudioMoth's −50 ppm crystal, about 4.3 s per day of unbroken capture — which
`stepped_by` cannot see, because it compares the system wall clock against the
system monotonic clock and the capture clock is a third clock, drifting rather
than stepping. "Caught by the instrument rather than by eye" is true of steps
only. And `oo_capture_clock_reanchors_total` reading 0 is not evidence that the
anchor was right: it counts steps observed *after* anchoring, so a station whose
network never comes up keeps a wrong anchor and a zero counter. What settles it
is comparing `systemd-timesyncd`'s first synchronisation against the stream's
`started_utc`, as [[SOAK_2026-08-22]] did.
