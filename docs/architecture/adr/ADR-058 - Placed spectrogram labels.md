...
- **The merge key is the rendered title, not a taxon id.** Two detections merge
  ...
  ([[ADR-044 - Withdrawn detections|ADR-044]] renders it `… · withdrawn`) can never be counted into a standing one,
...
**Reviewed 2026-08-29:** the decision holds, and the station is now running it: the
bundle served from `http://<station-host>:8080/` carries this module's `runText`
verbatim, down to `×${count} · best ${percent}%`. A real iOS or Android browser
is still unverified.

**Known and not addressed here.** Above the 640 px breakpoint [[ADR-054 - Responsive layout|ADR-054]] floats the
