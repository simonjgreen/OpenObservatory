...
**Status:** active; the "no Alembic environment exists" position in its final section is
superseded by [[ADR-035 - Alembic environment|ADR-035]], and the `ALTER TABLE` patcher that section describes was deleted
by [[ADR-042 - Migrations run in deploy.sh|ADR-042]] — see **Reviewed 2026-08-29** below.
...
this repository at all yet ([[ADR-007 - SQLite in developer mode|ADR-007]] already flags this gap for the SQLite→Postgres
...
> **Status 2026-08-08: superseded by [[ADR-035 - Alembic environment|ADR-035]].** An Alembic environment now
...
> [[ADR-035 - Alembic environment|ADR-035]] for why, and for what still has to change before it can be retired.

**Reviewed 2026-08-29:** the decision holds and is what the station serves: `GET /api/v1/history`
returns `suspect_stream_count`, and every stream span carries `claimed_seconds`,
`frame_derived_seconds` and `suspect` beside the windowed `seconds`. Two corrections to the
text above:

- **The `ALTER TABLE` patcher is gone.** The status note's "deliberately retained for now"
  was overtaken by [[ADR-042 - Migrations run in deploy.sh|ADR-042]] on 2026-08-09: `_patch_sqlite_columns` is deleted,
  `db/session.py: ensure_schema_at_head()` is the only production schema path, and
  `deploy/deploy.sh` runs `alembic upgrade head` before every restart. The migration this
  section records as still owed has been paid.
- **The frame-derived cap has an exception**, present since the original change but not
  described above: `history.py: _frame_count_is_trustworthy()` drops the frame bound for a
  row `_close_orphaned_streams` closed before the heartbeat existed
  (`end_reason='process_exited'` with no `detail.orphan_recovery.method == "heartbeat"`),
  because every such row carries `frame_count == 0` — never recorded, not measured. The
  `last_frame_at_utc` bound still applies to those rows, and `find_suspect_streams()` skips
