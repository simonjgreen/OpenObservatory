...
**Status:** active; the charter's "item 8 loses to items 1–2" precedent.
...
stalls [[ADR-033 - Retention is paced|ADR-033]] showed produce capture gaps. The heterodyne already carried this
...
correct for: the quantity [[ADR-033 - Retention is paced|ADR-033]] cares about is exactly how long the loop is
...
assumed.** [[ADR-038 - Display push channel|ADR-038]]'s channel and the debug UI's live channel share one process
...
[[ADR-012 - One writer per WebSocket|ADR-012]]'s single-writer rule and [[ADR-038 - Display push channel|ADR-038]]'s separate endpoint are what already
...
is inferred from [[ADR-033 - Retention is paced|ADR-033]]'s mechanism rather than demonstrated, because zero gaps
...
**Reviewed 2026-08-29:** the rollback no longer needs the restart. [[ADR-048 - Web-configurable settings|ADR-048]] made
`spectrogram_encode_min_viewers` a live-tier setting editable from the web UI
(`src/open_observatory/site_settings.py:493`), and the gate reads it fresh from
`Settings` on every block (`src/open_observatory/station.py:977`) rather than at
construction, so saving 0 there restores always-on encoding from the next block.
Editing `config/runtime.env` by hand still works and still needs a restart to be
read.
