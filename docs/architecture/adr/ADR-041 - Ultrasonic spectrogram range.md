...
**Status:** active; both values are unchanged and in force on the station. The
override route has widened -- [[ADR-048 - Web-configurable settings|ADR-048]] made the pair live-tier settings
...
unmeasured, since the channel was first built (`db30257`). Both values are
...
required, because [[ADR-040 - Spectrograms only when watched|ADR-040]] gates the encoders on there being a viewer):
...
**Reviewed 2026-08-29:** the revert no longer needs the restart. [[ADR-048 - Web-configurable settings|ADR-048]] made
both values live-tier settings editable from the web UI
(`src/open_observatory/site_settings.py:447`), pushed to the running encoder's
`retune()` through `tuning.LIVE_TARGETS` (`src/open_observatory/tuning.py:51`,
applied at `src/open_observatory/station.py:1441`), so a value saved there is in
force on the next block. Editing `config/runtime.env` by hand still works and
still needs a restart to be read.
