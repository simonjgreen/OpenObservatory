...
already has ([[ADR-038 - Display push channel|ADR-038]]), verifies a SHA-256 before anything becomes bootable, and
...
[[ADR-023 - The ESP32 inside observer|ADR-023]] kept the stock partition table byte for byte, for two reasons that are
...
- **Push over the existing channel.** A new frame type on [[ADR-038 - Display push channel|ADR-038]]'s socket
