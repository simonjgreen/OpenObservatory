...
**Status:** active; its startup-wiring follow-up ("`api/app.py` and `cli.py`
calling Alembic instead of `create_all()`") is closed by [[ADR-042 - Migrations run in deploy.sh|ADR-042]], and the
index its revision `0002` added was deliberately dropped again by [[ADR-062 - Retention walks live assets|ADR-062]]'s
revision `0011` — see **Reviewed 2026-08-29**.
...
per [[ADR-007 - SQLite in developer mode|ADR-007]].
...
**Done, by [[ADR-042 - Migrations run in deploy.sh|ADR-042]] (2026-08-09):** `deploy/deploy.sh` now runs `alembic
...
deleted. See [[ADR-042 - Migrations run in deploy.sh|ADR-042]] for why the migration itself runs in the deploy script
...
**Reviewed 2026-08-29:** the decision holds. `alembic/env.py` still resolves the
URL through `Settings().resolved_database_dsn` (`alembic/env.py:51`), targets
`Base.metadata` from `db/models.py` (`alembic/env.py:41`) and sets
`render_as_batch=True` in both the offline and online paths (`alembic/env.py:62`,
`alembic/env.py:83`); the revision chain is still linear from `0001_initial`.
Three things a reader of the above would otherwise get wrong:

- **The index revision `0002` exists to add is dropped again further up the
  chain, on purpose.** Revision `0011` ([[ADR-062 - Retention walks live assets|ADR-062]], 2026-08-19) drops
  `ix_media_asset_reclaimed_at` because SQLite preferred that low-selectivity
  index over the partial ones the retention sweep needs, and
  `tests/test_migrations.py` and `tests/test_retention.py` both assert it stays
  gone. `0002` is still correct as written and still runs on the way to `head`;
  what is wrong is to read this ADR as saying `head` carries that index.
- **The authentication follow-up landed** as revision `0003_auth_tables`
  (2026-08-08), generated and verified the way the paragraph above asks.
- **Two production entry points still call `create_all()`.** The "Done, by
  [[ADR-042 - Migrations run in deploy.sh|ADR-042]]" paragraph is true of what [[ADR-042 - Migrations run in deploy.sh|ADR-042]] itself converted, but
  `oo refine run` and `oo refine status` (`src/open_observatory/cli.py:1661`,
  `src/open_observatory/cli.py:1744`), added afterwards for [[ADR-045 - Refinement runner|ADR-045]], call
  `create_all()` rather than `ensure_schema_at_head()`, and
  `deploy/open-observatory-refine.service` runs the first of them unattended on
  the station. Those two skip the head check every other entry point performs.
  Tracked against [[ADR-042 - Migrations run in deploy.sh|ADR-042]], which owns that claim; not fixed here.
