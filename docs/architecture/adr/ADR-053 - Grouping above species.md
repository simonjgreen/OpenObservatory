...
**Status:** proposed. Nothing is implemented — re-confirmed 2026-08-29. Recorded because the question has
...
that is not an animal ([[ADR-049 - Sound categories are not species|ADR-049]]). It is not a taxonomic rank and must not be
...
2. **Family and order require a real taxonomic reference** — a checksummed,
   ...
   ([[ADR-006 - Model install and licensing|ADR-006]]), not a table typed into the source. eBird/Clements is the natural
...
**Relationship to [[ADR-043 - Taxon correction|ADR-043]].** [[ADR-043 - Taxon correction|ADR-043]] argued against introducing a taxonomy
