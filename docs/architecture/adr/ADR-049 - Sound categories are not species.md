...
**Status:** active. Corrects [[ADR-032 - Plausibility bands|ADR-032]]'s plausibility floor and [[ADR-044 - Withdrawn detections|ADR-044]]'s
...
1. **91 of the 114 findings were correct detections.** 62 `Engine`, 24 `Human
   ...
   taxon with a distribution — and [[ADR-032 - Plausibility bands|ADR-032]]'s floor reads that as "essentially
...
2. **The taxonomy was wrong at the source.** Every one of those 247 rows is
   ...
   **Corrected at merge.** An earlier draft of this ADR read the stored
   ...
   as evidence that [[ADR-032 - Plausibility bands|ADR-032]] was never deployed to the station. It was not: those
   ...
   counter that exists only under [[ADR-032 - Plausibility bands|ADR-032]] — was already at 19. [[ADR-032 - Plausibility bands|ADR-032]] is live
...
browser behind a `danger` acknowledgement ([[ADR-048 - Web-configurable settings|ADR-048]]). With it off, a detection
...
when a clip ages out ([[ADR-026 - Tiered clip retention|ADR-026]]), so `/api/v1/media/{id}` keeps answering 410
...
([[ADR-006 - Model install and licensing|ADR-006]] — the labels are never committed); it was run against the station's
...
**Reviewed 2026-08-29:** the check is in force (`normaliser.py:347`), and its
reach is narrower than "detector-agnostic" makes it sound. It fires only when
`rank == "species"`, so a taxonomic name carried at any other rank — including
`rank=None` — is never examined; and `_looks_like_a_binomial`
(`normaliser.py:44`) tests shape alone, so any well-formed binomial passes.
`NON_TAXONOMIC_PLUGINS` still holds only `activity-v1` (`normaliser.py:38`),
so `ultrasonic-pass-v1` has no plugin-level guard and this check is all that
stands behind it: were it to emit `rank='species'` with
`scientific_name='Nyctalus noctula'`, the normaliser would accept it and mint
`sci:nyctalus_noctula`, because `taxonomic_group='bat'` is not in
`NON_TAXONOMIC_GROUPS` either (`normaliser.py:41`). It does not. It emits
`label='bat pass'`, `rank=None`, `taxonomic_group='bat'` and no name at all,
keeping its species guess inside `native_result.candidate_name` next to
`hint_is_not_identification`
(`src/open_observatory/detectors/ultrasonic.py:525-560`). That restraint is
the detector's own; nothing in the normaliser enforces it.
...
detector and `plausibility_repair` share it, as [[ADR-032 - Plausibility bands|ADR-032]] intended — one
...
**Why this one rewrites typed columns when [[ADR-043 - Taxon correction|ADR-043]] says the original claim is
never edited and [[ADR-044 - Withdrawn detections|ADR-044]] marks rather than rewrites.** Both precedents are
...
([[ADR-043 - Taxon correction|ADR-043]] point 6) offers `sci:engine` as a taxon a reviewer can correct a real
bird *into*. So [[ADR-044 - Withdrawn detections|ADR-044]]'s binding rules are kept — nothing deleted, the
...
* **`Gray Wolf` is still filed under `taxonomic_group='bird'`.** `Canis lupus`
  ...
  taxonomy source, which [[ADR-043 - Taxon correction|ADR-043]] argued at length against introducing. One row
...
curl -s http://<station-host>:8080/api/v1/debug/pipeline \
  | python3 -c 'import json,sys; c=json.load(sys.stdin)["station"]["clips"]; print(c["skipped_human_audio"], c["policy"]["clip_human_audio"])'
```
