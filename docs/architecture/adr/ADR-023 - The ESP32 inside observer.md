...
**Status:** active on the no-score rule and the device choice; its HTTP-polling
transport is **superseded by [[ADR-038 - Display push channel|ADR-038]]** (push, with polling retained as an
exercised fallback), its premise that "no code in this repository publishes to a
broker" is **superseded by [[ADR-025 - MQTT and Home Assistant|ADR-025]]**, and the single-app-slot partition table
it froze is **superseded by [[ADR-050 - Display OTA slots|ADR-050]]**.

> **Transport superseded by [[ADR-038 - Display push channel|ADR-038]] (2026-08-09).** The display is now pushed to
...
> distinct failure states. [[ADR-038 - Display push channel|ADR-038]] tightens the score rule rather than relaxing
> it: the field no longer exists on the wire at all.
>
> [Corrected 2026-08-29: the partition table did **not** stay unchanged. Later the
> same day, [[ADR-050 - Display OTA slots|ADR-050]] replaced the stock single app slot and the unused SPIFFS
> partition with two equal OTA slots. NVS at `0x9000` is the part that survived —
> the inheritance this ADR cares about is intact — and the board and every honesty
> rule are untouched.]
...
> **Status 2026-08-08:** that premise no longer holds — [[ADR-025 - MQTT and Home Assistant|ADR-025]]'s MQTT publisher
...
everything the display needs — including the `include_synthetic` exclusion of [[ADR-020 - Non-live sources excluded|ADR-020]],
...
same interface without the UI changing. ([[ADR-038 - Display push channel|ADR-038]] is what that seam was for: a
...
with the station remaining the single authority on what local means. **[[ADR-038 - Display push channel|ADR-038]]
...
detects passes, not species ([[ADR-013 - ultrasonic-pass-v1|ADR-013]]). Its score is a pulse-train confidence about
...
The failure that motivates this is the same one behind [[ADR-020 - Non-live sources excluded|ADR-020]]. When the AudioMoth's
mode switch was moved [date corrected 2026-08-09: **2026-08-08**, matching [[ADR-020 - Non-live sources excluded|ADR-020]]
...
* Anything that lists detections as observations honours [[ADR-020 - Non-live sources excluded|ADR-020]]'s synthetic
  exclusion. The display requests `include_synthetic=false` explicitly rather than
  relying on the default.

**Reviewed 2026-08-29:** the honesty rules are all still enforced in running code.
`FeedItem` still has no field that could carry a score and
`test_no_feed_item_ever_carries_a_score` still guards it
(`firmware/inside-observer/test/test_feed/test_feed.cpp:228`); the push frame has
no score field either, and says so in a comment; the threshold is still presented
as the five named steps and never as a figure
(`firmware/inside-observer/src/display.cpp:118`); a bat pass still renders as
"Bat pass" with a peak frequency and no name; and both polling queries still send
`include_synthetic=false` explicitly
(`firmware/inside-observer/src/station_source.cpp:122,146`). `StationSource` is
still the seam, with `HttpStationSource` and `PushStationSource` behind it and the
poller genuinely woken when the socket stays down
(`firmware/inside-observer/src/main.cpp:804`). The three failure states are still
visibly distinct and still carry the copy quoted above — the banner renders the
station's own reason, "STATION UNREACHABLE" or "NO MICROPHONE - SYNTHETIC SOURCE",
and the footer still labels a surviving feed `(stale)`. An empty screen now also
carries a headline above that reason: "Station offline", "Not listening" or
"Nothing yet" (`firmware/inside-observer/src/display.cpp:502`). The partition
table is now history rather than description (see the correction in the header
note and [[ADR-050 - Display OTA slots|ADR-050]]), and with it "962 kB of the 3 MB app partition", since the
app slot is now 1,984 kB and there are two of them. The
"single reused 240x40 sprite" is now three — the row sprite was joined by a 72x18
elapsed-time sprite and a 52x18 WiFi indicator, each repainted on its own schedule
— but they are still small sprites for the same no-PSRAM reason, and no
framebuffer was allocated. One constraint has been added since, by
