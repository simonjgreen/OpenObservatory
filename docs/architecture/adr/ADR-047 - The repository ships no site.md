...
**Status:** active; widened by [[ADR-048 - Web-configurable settings|ADR-048]] from this site-identity whitelist to every field of `Settings`. The mechanism recorded here — `config/runtime.env` as the one store, the three tiers, `Station.applied_site` keeping "saved" and "in force" apart — is what [[ADR-048 - Web-configurable settings|ADR-048]] builds on unchanged.
