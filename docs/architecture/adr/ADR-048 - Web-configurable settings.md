...
**Status:** active; extends [[ADR-047 - The repository ships no site|ADR-047]]'s mechanism rather than replacing it. The
audit is still exhaustive and still enforced in code, and the deployed station
matches it. One count below has moved, and the audit's boundary is narrower than
"every setting" sounds — see the note at the end.
...
**Context.** [[ADR-047 - The repository ships no site|ADR-047]] established the mechanism — `config/runtime.env` as the
...
else, including every detector threshold and the [[ADR-041 - Ultrasonic spectrogram range|ADR-041]] spectrogram floors
...
- **Tiers.** *Live* means in force now. Most live settings are free — the
  ...
  not injected: coordinates ([[ADR-047 - The repository ships no site|ADR-047]]'s original reasoning — a live swap under a
...
- **Validation is the API's job, in two layers.** Per-field: type via Pydantic
  ...
  ceiling, ring at least two capture blocks ([[ADR-030 - ALSA ring and capture thread|ADR-030]]), retention ladder in
...
- **Defaults travel with the fields.** Every field carries its shipped value in
  ...
  it. The [[ADR-041 - Ultrasonic spectrogram range|ADR-041]] measurements are only a reference point if an operator who
...
`GET /api/v1/settings` there answers with the same 135 fields, 20 exclusions and
empty `pending_restart` as `main`. One count above has moved: `EDITABLE_SETTINGS`
holds 135 entries now, not 129 (`src/open_observatory/site_settings.py:237`), 98
live-tier and 37 restart-pinned. The exclusions are still exactly the twenty named
here (`src/open_observatory/site_settings.py:909`), the partition is still exact,
and `tests/test_site_settings.py::TestTheAuditIsComplete` still enforces it. The
change is the "For successors" obligation being met rather than dodged: seven
fields arrived with a tier declared — [[ADR-052 - Near-miss ledger|ADR-052]]'s `birdnet_near_miss_ring`,
[[ADR-055 - Timed recording pause|ADR-055]]'s pause presets, `capture_read_timeout_s`, `clip_human_audio` and
three more — and one, `retention_exemplar_only_days`, left with [[ADR-061 - Operator keep flag|ADR-061]].

**What the audit does not cover.** It partitions the fields of `Settings`, not the
keys of `config/runtime.env`. `coerce_updates` refuses an unknown key on write, on
the stated ground that silently dropping one "is how a UI bug looks like a saved
setting" (`src/open_observatory/site_settings.py:1106`); the read path does the
opposite. `Settings` is built `extra="ignore"` (`src/open_observatory/config.py:27`)
and [[ADR-047 - The repository ships no site|ADR-047]]'s store preserves lines outside the catalogue byte-for-byte by
design, so deleting a field from `Settings` — which the registry above handles
correctly — leaves its env line behind on every station that had it: inert, absent
from the settings payload, and looking live to anyone reading the file. The worked
example is `OO_RETENTION_EXEMPLAR_ONLY_DAYS`, which [[ADR-061 - Operator keep flag|ADR-061]] removed while
recording that the live station's file still sets it to 90. That ADR named the
hazard and deferred the fix as "a `Settings`-wide decision", which is this ADR's
ground: an unknown-key report belongs on this surface and is not on it yet.

### Rollback and smoke test (ADR-048)

No schema change, no new dependency, no migration. Additive to [[ADR-047 - The repository ships no site|ADR-047]]: revert
