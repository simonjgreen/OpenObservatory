...
**Status:** active; both defects are still fixed in `band_for`
(`detectors/birdnet.py`) and the four counters are live on the station. Three
later ADRs amend it without replacing it: [[ADR-044 - Withdrawn detections|ADR-044]] completed it, making the
consumers read the flag this ADR only wrote; [[ADR-049 - Sound categories are not species|ADR-049]] corrects the floor,
which must not be applied to BirdNET's eleven non-taxonomic classes; [[ADR-070 - Threshold retune is not a defect|ADR-070]]
fixed the repair command, which passed only the floor and re-judged history
against its own default band thresholds rather than the station's.
...
follows the exact shape of [[ADR-024 - Coverage bounded by frames|ADR-024]]'s `oo history reconcile-streams`: it never
...
detections — and why that differs from [[ADR-020 - Non-live sources excluded|ADR-020]]'s precedent.** [[ADR-020 - Non-live sources excluded|ADR-020]] keeps
...
**Reviewed 2026-08-29:** the decision holds and the code still matches it —
`band_for` (`src/open_observatory/detectors/birdnet.py:103`) returns
`implausible` at `math.inf` at or below `birdnet_plausibility_floor` (still
`0.0005`, `src/open_observatory/config.py:250`) and `no_prior` at the
out-of-range bar, and the live station exports the four scoped counters as
`oo_birdnet_suppressed_total{plugin_id,reason}`. Three things above have since
been overtaken. The consumers were built: [[ADR-044 - Withdrawn detections|ADR-044]] made the API, the MQTT
publisher and the counter-top display read `native_result.plausibility_review`
through one shared definition (`src/open_observatory/plausibility.py`), so a
flagged row is no longer "still visible everywhere it was before". The repair
command was run with `--apply` on the live station on 2026-08-09, before the
check this ADR asked for had been done; a later dry run showed it had been
