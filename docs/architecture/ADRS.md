...
| 042 | The refinement runner is a separate, CPU-fenced process, and the BatDetect2 cascade may only propose | active; promotes ADR-017's offline cascade to a scheduled job without adopting it as a source of station claims |
...
## ADR-042: The refinement runner is a separate, CPU-fenced process, and the BatDetect2 cascade may only propose
...
pre-ADR-042 behaviour. `OO_REFINEMENT_ENABLED=false` in `config/runtime.env` is
