...
- New ADRs get a new file **and** a row here. **Adding a body without adding an
  ...
  Both read **75** on 2026-08-30 and **76** on 2026-09-14. The two reserved gaps, 031 and 036, have a row
...
| [[ADR-078 - Database on the evidence SSD\|078]] | The database lives on the evidence SSD, and the station refuses to fork the record onto the SD card | accepted, 2026-09-14. Supersedes [[ADR-021 - Clips on their own device\|ADR-021]]'s *database stays on the SD card* paragraph and [[ADR-037 - Prune the dead indexes\|ADR-037]] option H. Argued from latency, not wear: `fdatasync` p50 11.3 ms on the card against 2.2 ms on the SSD, 66 ms mean per write request, and [[CAPTURE_RUN_2026-08-31]]'s 0.54% of detections lost to `database is locked`. Moves the file under the clips mount via `OO_DATABASE_DSN`; `OO_DATABASE_REQUIRE_MOUNT` makes a boot without the SSD a visible refusal rather than a second, empty record. Adds `oo db status/copy/check`; `copy` is `VACUUM INTO`, the first copy method verified against the live station |
