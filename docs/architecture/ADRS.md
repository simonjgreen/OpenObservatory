...
## ADR-024: A spacing/type scale for the web UI, applied to new surfaces rather than migrated wholesale
...
## ADR-025: Operator/diagnostic disclosure implemented as one depth toggle, not a second route
...
## ADR-026: Retention UI built against an assumed API shape; review workflow shipped minimally
