...
- ~~Whether 384 kHz capture is sustainable on this Pi/kernel/SD-card combination.~~ **RESOLVED over minutes; over days it FAILED.** Continuity 0.9990–0.9997 with zero gaps or overruns was the short-run figure. The 72-hour soak that would make this a durable claim ran 2026-08-10 to 2026-08-13 and came in at 99.865% against a ≥ 99.9% criterion — 349.3 s lost out of 259,200 s. See [[MILESTONE_STATUS]] §Milestone 4.5.
...
- ~~**No Alembic migration environment exists.**~~ **RESOLVED 2026-08-08 ([[ADR-035]]).** `alembic/` and `alembic.ini` exist with three revisions, wired to `Settings` and `Base.metadata`. The live station is at `0003_auth_tables (head)`. ~~**Still open within it:** nothing calls `alembic upgrade head` at startup or in `deploy.sh`, so `create_all()` and the `ALTER TABLE` patcher in `db/session.py` are deliberately retained.~~ **CLOSED ([[ADR-042]]).** `deploy/deploy.sh` now runs `alembic upgrade head` against the still-running old version before restart, and the `_patch_sqlite_columns` safety net no longer exists in `db/session.py`. There are now ten revisions; head is `0010_kept_at_partial_index`.
- ~~**The `review` table has no writer.**~~ **RESOLVED 2026-08-08 ([[ADR-029]]).** `POST`/`GET /api/v1/detections/{id}/review` exist and insert append-only rows, wired to confirm/reject in the detection drawer. ~~**Still open:** `corrected_taxon_id` is always written `None`; correcting a misidentified taxon is deliberately deferred.~~ **CLOSED ([[ADR-043]]).** `corrected_taxon_id`/`corrected_common_name`/`corrected_scientific_name` are now written when a reviewer supplies a correction.
