...
0. **North American owls are being reported at the development station, and they now reach a
   ...
   ADR-042 (consumers, plus the week audit). One operator action remains:
   ...
   **What is still open:**
   ...
   - ~~**No consumer hides a flagged historical row.**~~ **Done (ADR-042,
     ...
     *record* is marked, a *claim* is suppressed — is argued from charter items 5
     and 6 in ADR-042. **The firmware change is committed but NOT flashed: the
   ...
   - ~~**The week index passed to the range model was not re-audited.**~~
     **Done (ADR-042): it is correct.** Re-derived from the code and asserted for
