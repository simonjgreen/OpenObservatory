...
[[ADRS]] for the full reasoning behind any decision — this
...
measured figures are authoritative in [[TARGET_DIAGNOSTICS]];
what is done versus outstanding is authoritative in [[MILESTONE_STATUS]]; the
map of everything is [[README]]; setting up a dev environment is
[[SETUP]].
...
against a 259.2 s budget ([[SOAK_2026-08-22]]). Attempt 1
...
[[ACCEPTANCE_CRITERIA]] was exercised during the window.
...
([[ACCEPTANCE_CRITERIA]]). That is 0.006 pp of headroom: 175 s lost across the
...
See [[MILESTONE_STATUS]], which is the authority. The §1d reading of 99.906% at
...
fine. **A soak can pass every criterion in [[ACCEPTANCE_CRITERIA]] while
...
gain (see [[TARGET_DIAGNOSTICS]]). In that position it enumerates as USB ID
...
**[[TARGET_DIAGNOSTICS]] is the authoritative home for measured
...
4. ~~**Re-run the BatDetect2 benchmark and commit its output.**~~ **Done
   ...
   **It also turned up something the original gap was hiding.** The re-run is
   ...
   kept side by side in [[BATDETECT2_EVALUATION]] rather than one
...
4. **Reduce the AudioMoth gain.** The input still clips on loud nearby events. This
   ...
   layout) or do it by hand and record the new setting in [[TARGET_DIAGNOSTICS]].
...
7. **`oo audio window-dump`.** **Done, 2026-08-09.** Milestone 2 asked for a window
   ...
   [[DEPLOYMENT_AND_OPERATIONS]]'s CLI table. This closes the
