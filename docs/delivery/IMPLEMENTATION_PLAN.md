...
Revised 2026-08-05 against what Milestone 3 actually built. Per [[ADR-016 - Debug UI is the dashboard's foundation|ADR-016]], this milestone
...
Not foundation, despite appearances — see [[ADR-016 - Debug UI is the dashboard's foundation|ADR-016]] for the measurements:
...
- API token and authentication foundation, closing [[ADR-015 - Anonymous read, auth deferred|ADR-015]].
...
- native-rate evidence and audible playback rendering — time expansion and heterodyne,
  per [[ADR-014 - Ultrasonic rendered audible|ADR-014]], which is what makes a bat detection checkable by ear;
- `ultrasonic-pass-v1`, a pulse-train detector per [[ADR-013 - ultrasonic-pass-v1|ADR-013]]. It was never in this plan.
...
> [[ADR-045 - Refinement runner|ADR-045]] decided it is the *wrong* mechanism for the BatDetect2 cascade — its
...
The display OTA ([[ADR-050 - Display OTA slots|ADR-050]]) was justified on its own terms — every firmware change
...
- **no configuration step that requires a terminal or a text editor.** [[ADR-047 - The repository ships no site|ADR-047]]
  made site parameters runtime state and [[ADR-048 - Web-configurable settings|ADR-048]] put them in the browser; this
...
- ~~**over-the-air update of the counter-top display, triggered from the Pi**~~ —
  **DONE, 2026-08-09 ([[ADR-050 - Display OTA slots|ADR-050]]).** Two OTA app slots, a digest verified before
...
- **Taxonomic grouping above species ([[ADR-053 - Grouping above species|ADR-053]]).** Browse and aggregate by family
  ...
  *Corvus* detections and silently drops the Magpie and the Jay. Family needs a
  ...
  [[ADR-053 - Grouping above species|ADR-053]] records the reasoning, and refuses the hardcoded-species-list shortcut
  in writing.
...
- **Watch the model assets for updates.** Dependabot covers pip, npm, Docker and
  ...
  by `oo models fetch` against the checksummed `models/manifest.tsv` ([[ADR-006 - Model install and licensing|ADR-006]])
