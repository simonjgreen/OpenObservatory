...
**Supersedes in part:** [[ADR-026 - Tiered clip retention|ADR-026]] (tiered retention) — the exemplar mechanism only
...
[[ADR-060 - A stalled read is a dead stream|ADR-060]] made the wedge survivable. This removes the exposure that causes it.
...
([[ADR-043 - Taxon correction|ADR-043]], charter priority 5). Keeping a recording is a storage preference, not a
...
`deploy.sh` ([[ADR-042 - Migrations run in deploy.sh|ADR-042]]).
...
| A held detection is still exempt, and held ≠ kept | [[ADR-043 - Taxon correction\|ADR-043]]'s mechanism is untouched |
...
- The ESP32 display gets no keep control. It is a separate firmware deploy with
  its own OTA rollback drill ([[ADR-050 - Display OTA slots|ADR-050]]).
- The 15,704 bat detections never examined by a refiner ([[ADR-045 - Refinement runner|ADR-045]]) remain
