...
`src/open_observatory/config.py` on **2026-08-09**, after the [[ADR-045 - Refinement runner|ADR-045]]/049/052 work landed
on `main`.
...
to a **separate CPU-fenced process** that may only *propose* ([[ADR-045 - Refinement runner|ADR-045]]). See "The refinement
runner" at the end.
...
thing, running in a different process, at propose-only authority ([[ADR-045 - Refinement runner|ADR-045]]). See
[[BATDETECT2_EVALUATION]] for the figures, [[ADR-017 - BatDetect2 as an optional adapter|ADR-017]] for the not-a-live-detector decision,
...
no licence accepted — still exercises the window-to-detector-to-normaliser-to-event path.
**Not the clip stage:** `clip_plugins` has never listed `activity-v1` (`config.py:346`),
so a zero-action build gets its clip coverage from `ultrasonic-pass-v1`, which also needs
no model. Corrected 2026-08-29; the same overstatement was carried by ADR-010. It is a band-limited onset/energy segmentation detector with no model
...
input-level figures in [[TARGET_DIAGNOSTICS]]. [[ADR-010 - activity-v1, the first plugin|ADR-010]] constrains this detector to never
...
through the settings page ([[ADR-048 - Web-configurable settings|ADR-048]]) — `birdnet_threshold_in_range` among them. **The
...
[[ADR-032 - Plausibility bands|ADR-032]], sit outside that score-space ladder entirely: a species at or below
...
A sixth band, `non_biological`, was added by [[ADR-049 - Sound categories are not species|ADR-049]] and is checked **first**, before the
...
nothing about whether a car went past. Without this exemption [[ADR-032 - Plausibility bands|ADR-032]]'s floor was about to
withdraw a stack of true observations; the dry run that found it is in [[ADR-049 - Sound categories are not species|ADR-049]].
...
coordinates with
[[ADR-032 - Plausibility bands|ADR-032]]'s plausibility filtering switched on, and the test asserts both that "European
...
claim fields, and [[ADR-049 - Sound categories are not species|ADR-049]] added a generic per-detection backstop in the normaliser that
...
tests and [[ADR-013 - ultrasonic-pass-v1|ADR-013]] both note that 18–21 kHz is genuinely ambiguous between noctule and
bush-cricket, and the shipped code does not attempt to resolve that ambiguity.

Two honesty-relevant limitations, stated in [[ADR-013 - ultrasonic-pass-v1|ADR-013]] and in [[TARGET_DIAGNOSTICS]]:
...
`NON_TAXONOMIC_PLUGINS` guard does not cover it; [[ADR-049 - Sound categories are not species|ADR-049]]'s per-detection binomial backstop
...
still no BatDetect2 adapter, per [[ADR-017 - BatDetect2 as an optional adapter|ADR-017]], and this capability makes no assumption about
...
**And [[ADR-045 - Refinement runner|ADR-045]] decided it is not the mechanism for the BatDetect2 cascade either**, which
...
reached a consumer. Two further gaps [[ADR-045 - Refinement runner|ADR-045]] records and does not close: nothing connects a
...
Before [[ADR-032 - Plausibility bands|ADR-032]], `BirdNetDetector._band_for` sorted each candidate into `in_range`,
...
**And counters are not enough on their own ([[ADR-052 - Near-miss ledger|ADR-052]]).** Measured on the live station,
...
**The consumers do read that flag now ([[ADR-044 - Withdrawn detections|ADR-044]]).** `plausibility.py` holds the single
...
**The week index feeding all of this was audited and is correct ([[ADR-044 - Withdrawn detections|ADR-044]]):** 48 weeks,
...
this garden" but "a car has no distribution". [[ADR-032 - Plausibility bands|ADR-032]]'s plausibility floor read that as
...
(default off, [[ADR-048 - Web-configurable settings|ADR-048]]-editable) means such a detection now gets its row and no audio;
`oo clips purge-human-audio` removes what a station already holds, keeping the detection
rows. See [[ADR-049 - Sound categories are not species|ADR-049]] for the charter argument.
...
[[ADR-043 - Taxon correction|ADR-043]] deliberately did not introduce.
