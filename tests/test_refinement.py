"""The refinement runner, and the three charter rules it has to enforce (ADR-045).
...
class TestProposeOnly:
    ...
    def test_the_shipped_batdetect2_refiner_has_propose_authority(self) -> None:
        """If this ever flips, ADR-045 and the measured evidence must change first."""
...
class TestRetentionGap:
    def test_retention_still_deletes_on_age_alone(self, session_factory, tmp_path: Path) -> None:
        """Documented, not fixed. See ADR-045 "What this does not do".
        ...
        assert report.total_deleted > 0, (
            ...
            "has ever examined -- the gap ADR-045 records"
        )
