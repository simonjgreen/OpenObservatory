...
| 4 | **microSD card, 128 GB or larger, A2/V30** | 1 | £15 | OS and application only. The reference station uses a SanDisk 256 GB. The database lived here until 2026-09-14 (ADR-078); see the endurance warning below. |
| 5 | **USB SSD, 500 GB** | 1 | £45 | Evidence clips live here, mounted over `data/clips`, and since ADR-078 the SQLite database too, under `data/clips/database/`. Reference part: SanDisk Extreme Portable (`0781:558c`), USB/UAS. A busy bat night writes gigabytes; size it for your retention policy, not your first week. |
...
| **microSD only**, no SSD | Clips and database on one card. | Works, wears the card out, and the database's checkpoints stall behind the card's write latency (measured: an `fdatasync` is 11 ms on the card, 2 ms on the SSD). The reference station moved clips to an SSD for the first reason and the database for the second (ADR-021, ADR-078). |
...
- **A microSD is not a good home for a continuously-writing database.** This one
  was, until 2026-09-14, when measured lock contention moved it to the SSD
  (ADR-078). Put yours there from the start: `OO_DATABASE_DSN` in
  `config/runtime.env`, and `OO_DATABASE_REQUIRE_MOUNT=true` once it is.
