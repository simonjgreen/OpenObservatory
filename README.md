...
It started in a hammock, using the Merlin Bird ID app to work out what was
singing, and thinking it would be good to have that running all the time. Now it
does.
...
windows, and a detection that earns one is stored with a checksummed evidence
clip cut from the native ring buffer. It is local-first. Capture, detection,
review and query never need the internet.

A station is configured from its own web UI. It is watched from a browser, or
from an ESP32 counter-top display that the station updates over the air. It is
read through a REST/WebSocket API, an optional MQTT feed into Home Assistant,
and Prometheus.
...
forecaster, and the power supply is an iPad charger picked because it was known
to be good. The microphone is the only part bought for the job.

That is not incidental. A passive acoustic station does not need purpose-built
hardware. The interesting constraints here all follow from building it out of
what was to hand rather than out of a parts list - a 600 mA USB budget, a
database on an SD card, a display holding somebody else's WiFi credentials.
